...
# For highlight extrusion (hover pop)
...
@export var title_font: Font = SystemFont.new() ##DO NOT USE FontFile, it does not allow wrapping
...
@export var hint_font: Font = SystemFont.new() ##DO NOT USE FontFile, it does not allow wrapping
...
##TRIGGERING EXAMPLE COMMANDS WILL GRACEFULLY PRINT A HARMLESS ERROR, IGNORE IT.
...
# --- ANCHOR MINIMUM STABLE RADIUS FOR TEXT DOMAIN ---
...
func _ready() -> void:
	#THIS IS A TEST TO PROVE THAT IT WORKS INGAME,
	#BUT YOU CAN SET IT TO ANYTHING FROM ANYWHERE
	set_menu_construct(MenuBus.menu_construct_test)
	
	#THIS IS JUST KINDA NECESSARY
	prep_state()
...
func _process(delta: float) -> void:
	...
	# Time-to-target lerp for ring size
	...
	# Time-to-target lerp for outer ring extrusion
	...
	# --- FIX: Bulletproof extrusion: reset and animate only for the hovered segment ---
	if _current_menu_state == MenuState.INNER_RING_UNSELECTED:
	...
	# Time-to-target lerp for highlight extrusion
	...
	# --- Hover detection and context update (unchanged) ---
	var mouse_local_position = get_local_mouse_position()
	var mouse_vector = mouse_local_position - center_position
	var mouse_distance = mouse_vector.length()
	var center_back_radius = _inner_ring_inner_edge * center_back_margin_fraction
...
func _execute_command(method_name: String, data: Dictionary = {}) -> void:
	...
	if global_commands:
		if global_commands.has_method(method_name):
