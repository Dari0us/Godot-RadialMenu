...
# --- Exported parameters and colors ---
...
@export var menu_padding_unit: float = 25.0
...
# --- State ---
var highlight_extrusion: float = 10.0
...
# --- MetaStructure helpers ---
func _get_menu_categories() -> Dictionary:
	return menu_construct.get("menu_construct", {})

func _get_meta_source():
	return menu_construct.get("meta_source", null)

func _get_inner_segment_key(index: int) -> String:
	var keys = _get_menu_categories().keys()
...
func _get_outer_segment_key(index: int) -> String:
	...
	var sub_items = _get_menu_categories().get(cat_key, {}).get("sub_items", {})
	var keys = sub_items.keys()
	if index >= 0 and index < keys.size():
		return keys[index]
	return ""
...
func _update_outer_segment_keys() -> void:
	...
	if _selected_inner_segment_index != -1 and _selected_inner_segment_index < _inner_segment_keys.size():
		...
		var category_data = _get_menu_categories().get(selected_category_key, {})
		if category_data.has("sub_items"):
...
func update_context_elements():
	...
	if _current_menu_state == MenuState.INNER_RING_UNSELECTED:
		if _hovered_inner_segment_index >= 0:
			...
			context_title = key
			context_description = cats.get(key, {}).get("description", "")
			context_icon = _loaded_icon_textures.get(cats.get(key, {}).get("icon", ""), null)
		else:
			context_title = ""
			context_description = ""
			context_icon = null
		...
		if _hovered_outer_segment_index >= 0:
			...
			context_description = cats.get(cat_key, {}).get("sub_items", {}).get(out_key, {}).get("description", "")
			context_icon = _loaded_icon_textures.get(cats.get(cat_key, {}).get("sub_items", {}).get(out_key, {}).get("icon", ""), null)
...
func _draw() -> void:
	...
	if _current_outer_ring_outer_edge > _outer_ring_inner_edge + 0.1 and _current_num_outer_segments > 0:
		for i in range(_current_num_outer_segments):
			...
			var full_icon_path = ""
			if outer_item_key != "":
				var category_data = _get_menu_categories().get(_get_inner_segment_key(_selected_inner_segment_index), {})
	...
	for i in range(num_inner_segments):
		...
		var full_icon_path = ""
		if inner_item_key != "":
			var item_data = _get_menu_categories().get(inner_item_key, {})
			full_icon_path = item_data.get("icon", "")
		if full_icon_path != "" and _loaded_icon_textures.has(full_icon_path):
			var icon_texture = _loaded_icon_textures[full_icon_path]
			var mid_angle = (draw_start_angle + draw_end_angle) / 2.0
...
func _execute_command(method_name: String, data: Dictionary = {}) -> void:
	var global_commands = MenuBus
	var meta_source = _get_meta_source()
	var meta_input = null
	var cats = _get_menu_categories()
	if data.has("ring"):
		if data["ring"] == "inner":
			var item_data = cats.get(data["key"], {})
			meta_input = item_data.get("meta_input", null)
		elif data["ring"] == "outer":
			var cat = cats.get(data["category_key"], {})
			var item_data = cat.get("sub_items", {}).get(data["key"], {})
			meta_input = item_data.get("meta_input", null)
	if global_commands and global_commands.has_method(method_name):
		var method_info = null
		for m in global_commands.get_method_list():
			if m["name"] == method_name:
				method_info = m
				break
		var arg_count = 0
		if method_info and method_info.has("args"):
			arg_count = method_info["args"].size()
		# Now call with the right number of arguments
		if arg_count == 0:
			global_commands.call(method_name)
		elif arg_count == 1:
			global_commands.call(method_name, meta_source)
		else: # 2 or more
...
func _process(delta: float) -> void:
	...
	# Inner/outer highlight extrusion
	if _current_menu_state == MenuState.INNER_RING_UNSELECTED:
	...
	# Hover detection and context update
	var mouse_local_position = get_local_mouse_position()
	var mouse_vector = mouse_local_position - center_position
	var mouse_distance = mouse_vector.length()
	var center_back_radius = _inner_ring_inner_edge * center_back_margin_fraction
		...
		if _current_menu_state == MenuState.INNER_RING_UNSELECTED:
			if _hovered_inner_segment_index != -1:
				...
				if item_data.has("sub_items") and item_data["sub_items"].size() > 0:
					...
					_execute_command(item_data["command"], {"ring": "inner", "key": inner_item_key, "index": _hovered_inner_segment_index})
					_selected_inner_segment_index = -1
					_selected_outer_segment_index = -1
					_update_outer_segment_keys()
					queue_redraw()
			...
			if _hovered_inner_segment_index != -1:
				...
				if command_method_name != "":
					_execute_command(command_method_name, {"ring": "outer", "category_key": _get_inner_segment_key(_selected_inner_segment_index), "key": outer_item_key, "index": _selected_outer_segment_index})
				_update_outer_segment_keys()
				queue_redraw()
			elif is_mouse_in_center_back:
				_selected_inner_segment_index = -1
